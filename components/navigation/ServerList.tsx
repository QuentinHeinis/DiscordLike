...
const ServerList = ({servers} : {servers:Server[]}) => {  
  ...
  return (
    <>
      <ReactSortable list={navList} setList={setNavList} fallbackTolerance={4} className='flex flex-col gap-3 pt-3 mt-3 border-t-2 border-neutral-400 '>
      ...
      <button onClick={()=>setModalOpen('addServer')} className='bg-neutral-500 text-white h-14 w-14 rounded-full grid place-content-center mt-3'><PlusIcon className='h-8 w-8'/></button>
    </>
  )
}
