...
const ServerMember = ({members, isOwner, userId, server}:ServerMemberType) => {
  ...
  return (
    <div className="p-2 flex flex-col gap-1">
      ...
      {
      members.map((member) => (
        <div key={member.id} className="group flex justify-between hover:bg-neutral-400 dark:hover:bg-neutral-600 text-neutral-800 dark:text-neutral-300  rounded-md">
          <Link href={`/messages/${member.user.id}`} className="flex items-center gap-3 px-2 py-4 w-full">
            ...
            {!member.user.imageUrl && <div className="w-8 h-8 bg-indigo-700 rounded-full flex items-center justify-center text-white font-semibold text-xl">{member.user.name.charAt(0)}</div>}
            {member.user.name}
          </Link>
          {
