  ...
  return (
    <div className="group flex-none relative w-full bg-neutral-600 flex justify-around items-center h-16 border-b border-neutral-200">
      ...
      <div className="h-0 z-50 group-hover:h-fit group-hover:visible pt-2 pb-2 transition-all scale-50 invisible flex gap-1 flex-col items-center group-hover:scale-100 overflow-hidden absolute w-[95%] bg-neutral-500 top-full border-t">
        ...
        {isModo && (
          <>
            <button
              className="w-5/6"
              onClick={() =>
                setModalOpen("updateServer", {
                  server: server,
                  other: { name: serverName },
                })
              }
            >
              <ServerUtils
            ...
            <button
              className="w-5/6"
              onClick={() => setModalOpen("addChannel")}
            >
              <ServerUtils
                text="Créer un salon"
                icon={<PlusCircleIcon className="h-4" />}
              />
            </button>
          </>
        )}
        {isOwner || (
          <button
            className="w-5/6 text-red-500"
            onClick={() => setModalOpen("leaveServer", { server: server })}
          >
            <ServerUtils
              text="Quitter le server"
              icon={<LeaveIcon className="h-4" />}
            />
          </button>
        )}
      </div>
